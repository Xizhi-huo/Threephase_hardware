...
def simulate_dg_hess_ode(p, hess_params, csv_load_data=None):
    ...
    # 初始化负载基线值（使用t=0预加载值作为基线）
    P_load_base = initial_load_value
    ...
    # 每0.2秒取一个数据点
    interval = 0.2
    interval_step = max(1, int(interval / dt))
    interval_indices = np.arange(0, len(P_load_original), interval_step)
    interval_data = P_load_original[interval_indices]
    
    # 计算每个时刻的实时平均值（过去100秒，最多500个点，不使用未来数据）
    for i in range(len(P_load_original)):
        ...
        interval_end_idx = np.searchsorted(interval_indices, i, side='right')
        # 计算平均值，最多取过去500个点
        if interval_end_idx > interval_start_idx:
            start_avg_idx = max(interval_start_idx, interval_end_idx - 500)
            P_load_average_real_time[i] = np.mean(interval_data[start_avg_idx:interval_end_idx])
        else:
    ...
    for i in range(0, len(t)):
        ...
        # =========================
        # V/Hz控制只由界面开关决定
        vhz_enabled = bool(p.get("enable_voltage_speed_control", False))
        
        if vhz_enabled:
            # 计算频率
            rpm_base = p.get("rpm_base", 1500.0)
            generator_poles = max(2.0, p.get("generator_poles", 4.0))
            f_nom = rpm_base * generator_poles / 120.0  # 标准频率（Hz）
            f_actual = omega * rpm_base * generator_poles / 120.0  # 实际频率（Hz）
            ...
            if abs(f_deviation) > frequency_deadband:
                ...
                # 平滑过渡到目标电压
                vhz_response_time = 0.1
                vhz_alpha = min(1.0, dt / max(vhz_response_time, dt))
                Vref = min(1.0, max(0.8, Vref + (Vref_target - Vref) * vhz_alpha))
            else:
                # 频率在死区内，电压恢复到基础值
                vhz_response_time = 0.1
                vhz_alpha = min(1.0, dt / max(vhz_response_time, dt))
                Vref = min(1.0, max(0.8, Vref + (1.0 - Vref) * vhz_alpha))
...
class DG_HESS_SimulatorApp:
    ...
    def load_csv_load(self):
        ...
        if file_path:
            try:
                ...
                # CSV负载数据按输入值使用，load_pu参数负责整体缩放
                self.csv_load_data = np.column_stack((time_data, load_data))
    ...
    def on_slider_change(self, value, key, min_val, max_val, is_hess=False):
        try:
            ...
            if key == "generator_poles":
                val = max(min_val, min(max_val, round(val / 2.0) * 2.0))
            if is_hess:
    ...
    def create_parameter_controls(self):
        ...
        param_groups = {
            ...
            "基值": [
                ...
                ("generator_poles", "generator_poles", 2.0, 12.0, 2.0, "%.0f"),
    ...
    def reset_to_defaults(self):
        ...
        if hasattr(self, 'x_max_var'):
            self.x_max_var.set(self.params["t_end"])
        
        # 复位HESS参数
        self.hess_params = HESS_Params()
        for key, var in self.hess_param_vars.items():
            if hasattr(self.hess_params, key):
                var.set(getattr(self.hess_params, key))
    ...
    def create_plot_area(self):
        ...
        self.x_max_var = tk.DoubleVar(value=self.params.get("t_end", 70.0))
    ...
    def on_motion(self, event):
        ...
        try:
            if self.dragging and self.drag_start is not None and self.drag_annotation is not None and event.xdata is not None and event.ydata is not None:
                ...
                # 更新垂直线位置
                self.vertical_lines[self.current_line_index].set_xdata([event.xdata, event.xdata])
                self.vertical_lines2[self.current_line_index].set_xdata([event.xdata, event.xdata])
    ...
    def update_plots(self, t, P_load_original, P_load_hess, P_storage_total,
        ...
        # 保存用户添加的时刻线位置，重绘后恢复
        saved_vertical_times = []
        if hasattr(self, 'vertical_lines'):
            for line in self.vertical_lines:
                try:
                    saved_vertical_times.append(line.get_xdata()[0])
                except:
                    pass
        ...
        # 恢复用户添加的时刻线
        colors = ['red', 'blue', 'green', 'purple']
        for line_time in saved_vertical_times[:4]:
            if len(t) > 0 and t[0] <= line_time <= t[-1]:
                line_color = colors[len(self.vertical_lines) % len(colors)]
                line = self.axs2[0].axvline(x=line_time, color=line_color, linestyle='--', linewidth=1.5, label=f'时间线{len(self.vertical_lines)+1}')
                self.vertical_lines.append(line)
                line2 = self.axs2[1].axvline(x=line_time, color=line_color, linestyle='--', linewidth=1.5, label=f'时间线{len(self.vertical_lines)}')
                self.vertical_lines2.append(line2)
